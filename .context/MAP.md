...
| host | non-destructive AGENTS/CLAUDE marker-block entrypoint installation | `src/host`, `src/commands/install.ts` | `.context/modules/host.md` | install, AGENTS, CLAUDE, host, 入口 |
| skill | project-local skill pack export and bootstrap onboarding for IDE agents | `src/commands/skill.ts`, `src/commands/bootstrap.ts`, `src/skill` | `.context/modules/skill.md` | skill, bootstrap, IDE, start-here, 技能包, 接入 |
...
| AGENTS、CLAUDE、宿主入口、host、非破坏式安装、marker merge | host | `.context/modules/host.md`, `src/host/entrypoint-template.ts`, `src/host/merge-entrypoint.ts` |
...
- `host` generates and non-destructively merges marker-block entrypoint text used by `install`.
- `skill` exports project-local skill/reference packs and composes `context` plus `host` in `bootstrap` onboarding.
...
- Generated onboarding guide: `.context/out/start-here.md`
