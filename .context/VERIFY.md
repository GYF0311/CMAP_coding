...
| freshness / drift | `pnpm test tests/integration/m18-freshness-inbox-promote.test.ts`, `pnpm test tests/integration/m26-drift.test.ts`, and `pnpm test tests/integration/m30-freshness-lock.test.ts` | Freshness snapshots, semantic review markers, generated/canonical UX, locked atomic writes, read-only drift check, explicit drift sourceSignals snapshot, generated evidence drift, low-risk inbox promote apply, and explicit reject archive are covered. |
...
| HTML view | `pnpm test tests/integration/m19-view-export.test.ts` | View include flags, `--include-support`, `--ui-lang zh-CN`, Overview/Verification parsing, canonical context browsing, normalized HTML check, escaping/redaction, and size caps are covered. |
| HTML view interactions | `pnpm test tests/integration/m19-view-export.test.ts` | Search, stale/high-risk/generated filters, copy-command buttons, and structured module detail dialog hooks render without unsafe `innerHTML` or `eval`. |
