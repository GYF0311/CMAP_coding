...
export function renderViewHtml(data: CmapViewData, options: RenderViewOptions = {}): string {
  ...
  const supportSections = [
    safeData.included.freshness
      ? `<section><h2>${escapeHtml(messages.freshness)}</h2>${renderFreshness(safeData, messages)}</section>`
      : "",
    safeData.included.generated
      ? `<section><h2>${escapeHtml(messages.generatedEvidence)}</h2>${renderEvidence(safeData, messages)}</section>`
      : "",
    safeData.included.inbox
      ? `<section><h2>${escapeHtml(messages.reviewCandidates)}</h2>${renderCandidates(safeData, messages)}</section><section><h2>${escapeHtml(messages.relationCandidates)}</h2>${renderRelationCandidates(safeData, messages)}</section>`
      : "",
  ]
    .filter(Boolean)
    .map((section) => `    ${section}`)
